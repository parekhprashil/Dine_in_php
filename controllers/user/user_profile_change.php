    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST')
    {
        if (isset($_POST['tag']) && $_POST['tag'] === 'tag')
        {
            ...
            )
            {
                ...
                // check user data is already their or not
                $find_user = "SELECT * from `usermaster` WHERE user_id = ?";
                
                $find_user_stmt = mysqli_prepare($conn, $find_user);

                mysqli_stmt_bind_param($find_user_stmt, "i", $user_id);

                mysqli_execute($find_user_stmt);
                ...
                // {
                //     $user_details = mysqli_fetch_assoc($show_user_data_res   ult);
